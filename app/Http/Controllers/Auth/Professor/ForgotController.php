...
use App\Http\Requests\Auth\Professor\ForgotRequest;
...
class ForgotController
{
    ...
    {
        return view('auth.professor.forgot');
    }
    ...
    public function forgot()
    {
        if (Session::get('forgot.time') != false && Session::get('forgot.time') > time()) {
            ...
            $inputs = $request->all();
            $user = Professor::where('email', $inputs['email'])->get();
            if (empty($user)) {
            ...
            <p style="font-size: 15px; font-family: tahoma;">استاد گرامی برای بازیابی رمز عبور خود از لینک زیر استفاده نمایید</p>
            <p style="display: inline-block; padding: 5px 10px; background: #218838; border-radius: 3px;">
                <a style="color: #fff;" href="' . route('auth.professor.reset-password.view', [$user->remember_token]) . '">بازیابی رمز عبور</a>
            </p>
            ';
